...
public class BluetoothLeService extends Service {
    ...
    private void checkGattServices(List<BluetoothGattService> gattServices) {
        ...
        for (BluetoothGattService gattService : gattServices) {
            if (UUIDDatabase.UUID_INSTA360_CONTROL_SERVICE.equals(gattService.getUuid())) {
                uuid = gattService.getUuid().toString();
                Log.d(TAG, "Insta360 Control Service Found: " + uuid);
                gattCharacteristics = gattService.getCharacteristics();
                // Loops through available Characteristics.
                for (BluetoothGattCharacteristic gattCharacteristic : gattCharacteristics) {
                    uuid = gattCharacteristic.getUuid().toString();
                    Log.d(TAG, "Characteristic Found: " + uuid);
                    if (UUID.fromString(GattAttributes.INSTA360_COMMAND_CHARACTERISTIC).equals(gattCharacteristic.getUuid())) {
                        Log.d(TAG, "Insta360 Command Characteristic Found: " + uuid);
                        commandCharacteristic = gattCharacteristic;
                    }
                    if (UUID.fromString(GattAttributes.INSTA360_COMMANDRESPONSE_CHARACTERISTIC).equals(gattCharacteristic.getUuid())) {
                        Log.d(TAG, "Insta360 Command/Response Characteristic Found: " + uuid);
    ...
    public void requestCameraWifi(){
        if (commandCharacteristic != null) {
            if (!isNotifying(commandResponseCharacteristic)) {
                ...
                byte[] command = {0x16, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x08, 0x00, 0x02, 0x00, 0x00, 0x00, (byte)0x80, 0x00, 0x00, 0x0a, 0x04, 0x00, 0x00, 0x24, 0x00};
